...
//        }
        ...
        Statement statement = CCJSqlParserUtil.parse("SELECT DISTINCT ON (colg) s, SUM(col1,clo3,a) AS a, COUNT(col2) AS b, col3 AS c , col4 as d, col5 , col99 " +
                " FROM table WHERE col1 = 10 AND col2 = 20 XOR col3 = 30 AND col5 = 99");
                                ...
                                public void visit(SelectExpressionItem item) {
                                    if(item.getExpression() instanceof Column) {
                                        ...
                                        if(item.getAlias() == null){
                                            ...
                                            System.out.println(item.getAlias().getName() + " " + ((Column) item.getExpression()).getName(false));
                                            selectListItemMapWithAliases.put(item.getAlias().getName(), ((Column) item.getExpression()).getName(false));
                                        }
                                    }
                                    else if(item.getExpression() instanceof Function){
